...
void GameObjectManager::ProcessMove(GameObject* player, unsigned char dirType)
{
	...
	unordered_set <GameObject*> newViewList;
	for (int i = 0; i < NUM_OF_PLAYER; ++i) {
		if (false == playerArray_[i]->isActive_) continue;
		if (false == playerArray_[i]->CanSee(player)) continue;
		// 새로운 뷰리스트 (나를 기준)
		newViewList.insert(playerArray_[i]);
	}
	for (int i = 0; i < NUM_OF_NPC_TOTAL; ++i) {
		if (false == npcArray_[i]->CanSee(player)) continue;
		newViewList.insert(npcArray_[i]);
		WakeUpNPC(npcArray_[i], player);
	}

	for (auto& object : newViewList) {
